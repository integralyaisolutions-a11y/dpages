...
/**
 * Archivo separado a propósito: necesita mockear `undici` ANTES de
 * importar cliente.ts (que llama a `setGlobalDispatcher` como efecto de
 * módulo, una sola vez al cargarse). Mezclar esto con
...
describe('cliente.ts — configura connectTimeout de undici al cargarse (capa 49bis/49ter)', () => {
  ...
  it('llama a setGlobalDispatcher con un Agent cuyo connectTimeout = TIMEOUT_CONEXIO_MS (15_000), NO el mismo valor que TIMEOUT_PETICION_MS', async () => {
    ...
    // El diagnóstico real (ver docs/hallazgos-woocommerce.md):
    // AbortSignal.timeout() NO controla la fase de conexión — undici tiene
    // su propio connectTimeout (default 10_000ms), independiente. Usar el
    // MISMO valor que TIMEOUT_PETICION_MS rompía el diagnóstico — acá se
    // confirma que quedó un valor PROPIO y distinto (15_000, menor que los
    // 30_000 de la petición completa), no el mismo número reciclado.
