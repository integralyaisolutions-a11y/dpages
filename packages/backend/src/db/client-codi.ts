 ...
 * (`transform/resolucio-client.ts`, `resolverOCrearClient`) y el alta
 * manual (`POST /clients`) — las dos son, estructuralmente, "insertar una
 * fila de client y resolverle un codi después", porque `id_seq` (columna
 * IDENTITY) recién se conoce DESPUÉS del INSERT, nunca antes ni dentro de
 * la misma sentencia.
 *
 * Un ancho fijo (ej. zero-padded a 3 dígitos) trunca en vez de ensanchar en
 * cuanto `id_seq` llega a 4 cifras — id_seq 4916/4918 colisionarían en el
 * mismo "CLI491", violando `idx_client_codi`. No usar padding: sin ancho
 * fijo, nunca hay nada que truncar.
 *
