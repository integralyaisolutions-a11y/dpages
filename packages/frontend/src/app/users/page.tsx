  ...
  return (
    <DataCard>
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        ...
        <Badge variant={user.actiu ? 'positive' : 'negative'}>
    ...
    <div>
        ...
        action={
          tab === 'usuaris'
            ? {
                ...
                onClick: () => setUserFormState({ mode: 'create' }),
              }
            : {
                label: 'Nou rol',
                icon: <Plus className="h-4 w-4" />,
                onClick: () => setRoleFormState({ mode: 'create' }),
              }
        }
      />
        ...
        <>
          ...
          {usersError && (
            <p className="text-sm text-red-600">
              No s&apos;han pogut carregar els usuaris: {usersError.message}
            </p>
          )}
          ...
          {!usersLoading && !usersError && (
            <>
              <div className="flex flex-col gap-3 md:hidden">
                {filteredUsers.map((user) => (
                  <UserCard
                    key={user.id}
                    user={user}
                    onEdit={() => setUserFormState({ mode: 'edit', user })}
                  />
                ))}
              </div>
                ...
                <table className="w-full table-fixed text-sm">
                  <thead className="border-b border-gray-200">
                    <tr>
                      ...
                      <th className="w-[28%] px-3 py-2 text-left font-medium text-gray-500 break-words">
                        Email
                      </th>
                      <th className="w-[22%] px-3 py-2 text-left font-medium text-gray-500 break-words">
                        Rol
                      </th>
                      <th className="w-[16%] px-3 py-2 text-left font-medium text-gray-500 break-words">
                        Estat
                      </th>
                      <th className="w-[12%] px-3 py-2 text-right font-medium text-gray-500 break-words">
                        Accions
                      </th>
                    </tr>
                  </thead>
              ...
              {usersPaginacio && (
                <Pagination paginacio={usersPaginacio} onPageChange={setUsersPagina} />
              )}
            </>
          )}
        </>
      ...
      {tab === 'rols' && (
        <>
          ...
          {rolesError && (
            <p className="text-sm text-red-600">
              No s&apos;han pogut carregar els rols: {rolesError.message}
            </p>
          )}
          ...
          {!rolesLoading && !rolesError && (
            <>
              <div className="flex flex-col gap-3 md:hidden">
                {roles.map((role) => (
                  <RoleCard
                    key={role.id}
                    role={role}
                    onEdit={() => setRoleFormState({ mode: 'edit', role })}
                  />
                ))}
              </div>
              ...
              {rolesPaginacio && (
                <Pagination paginacio={rolesPaginacio} onPageChange={setRolesPagina} />
              )}
            </>
          )}
        </>
