    ...
    <div>
      ...
      {error && (
        <div className="flex items-center gap-3 rounded-lg border border-red-200 bg-red-50 px-4 py-3">
          <p className="text-sm text-red-600">No s&apos;han pogut carregar els transportistes: {error.message}</p>
          <button
            type="button"
            onClick={refetch}
          ...
          <div className="hidden overflow-x-auto rounded-xl border border-gray-200 bg-white xl:block">
            <table className="w-full table-fixed text-sm">
              <thead className="border-b border-gray-200">
                <tr>
                  <th className="w-[45%] px-3 py-2 text-left font-medium text-gray-500 break-words">Nom</th>
                  <th className="w-[25%] px-3 py-2 text-left font-medium text-gray-500 break-words">Codi</th>
                  <th className="w-[15%] px-3 py-2 text-left font-medium text-gray-500 break-words">Actiu</th>
                  <th className="w-[15%] px-3 py-2 text-right font-medium text-gray-500 break-words">Accions</th>
                </tr>
              </thead>
              <tbody>
                {data.map((carrier) => (
                  <tr key={carrier.id} className="border-b border-gray-100 last:border-0">
                    ...
                    <td className="px-3 py-3 break-words text-gray-900">{carrier.actiu ? "Sí" : "No"}</td>
