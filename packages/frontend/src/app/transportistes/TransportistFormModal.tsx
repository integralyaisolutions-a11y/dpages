...
export function TransportistFormModal({
  ...
  async function handleSave() {
    ...
    setIsSaving(true);
    try {
      await onSave({ nom: trimmedName, codi: codi.trim() || null });
    } catch (caught) {
      if (caught instanceof ApiError) {
        ...
        if (Object.keys(nextFieldErrors).length > 0) {
          ...
          // 409 de codi duplicat (esViolacioCodiUnic al backend) no ve amb
          // `detalls` per camp — mateix criteri que Categories/Tarifes/
          // Clients: cau al missatge genèric del formulari.
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={mode === "create" ? "Nou transportista" : "Modificar transportista"}>
